...
growth rate starts to decrease at a certain point.
...
value of β1 is small, from 1 to 3. However, two values are approximately
equal when true value is relatively higher, from 5 to 6. As true value
(effect size) increases, power increases and we are more likely to
reject the null. Therefore, We have larger sample size when true value
of β1 is high, as shown below. Given that estimates follow a Normal
distribution, the sample mean provides a good estimate as sample size
increases, so the mean of estimates will be closer to the true value.
